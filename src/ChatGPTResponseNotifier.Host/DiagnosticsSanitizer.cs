...
internal static class DiagnosticsSanitizer
{
    ...
    public static JsonElement Event(JsonElement input)
    {
        var safe = new
        {
            ...
            osBuild = IntegerValue(input, "osBuild"),
            candidateCount = IntegerValue(input, "candidateCount"),
            virtualDesktopSupported = BooleanValue(input, "virtualDesktopSupported"),
            desktopIdSuffix = StringValue(input, "desktopIdSuffix", 8),
