...
async function waitForExtensionAbsent(browser, extensionId, timeoutMs = 8000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if (!(await browser.extensions()).has(extensionId)) return true;
    await new Promise(resolve => setTimeout(resolve, 100));
  }
  return false;
}
...
try {
  ...
  // Exercise the exact one-shot production-safe route: ask the already-running
  // Chrome process for this disposable user-data directory to uninstall only
  // the stale ID. No DevTools attachment to a real profile and no preference
  // file mutation is used.
  const command = spawnSync(
    browserExecutablePath,
    [`--user-data-dir=${profilePath}`, `--uninstall-extension=${legacyId}`],
    { encoding: 'utf8', timeout: 15000, windowsHide: true },
  );
  assert(!command.error, `chrome-cli-launch-failed:${command.error?.message ?? 'unknown'}`);
  assert(command.status === 0, `chrome-cli-exit:${command.status ?? 'null'}`);

  const legacyRuntimeRemoved = await waitForExtensionAbsent(browser, legacyId);
  const stableRuntimeAfterCleanup = Boolean((await browser.extensions()).get(stableId));
  assert(legacyRuntimeRemoved, 'legacy-runtime-remained-after-cli-uninstall');
  assert(stableRuntimeAfterCleanup, 'stable-runtime-lost-during-cli-uninstall');
  ...
  const evidence = {
    schemaVersion: 1,
    capability: 'chrome-legacy-registration-cli-cleanup-disposable-v1',
    ...
    cleanupMethod: 'chrome-command-line-uninstall-existing-disposable-profile',
    browserVersion,
    stableId,
    legacyId,
    commandExitStatus: command.status,
    legacyRuntimeRemoved,
    stableRuntimeAfterCleanup,
    legacyAfter,
    stableAfter,
  ...
  console.log(`LEGACY_CLI_CLEANUP browser=${browserVersion}; legacy=${legacyId}; stable=${stableId}; exit=${command.status}; legacyRuntimeRemoved=${legacyRuntimeRemoved}; legacyPresentAfter=${legacyAfter.present}; stablePresentAfter=${stableAfter.present}; stableRootAfter=${stableAfter.sameDisposableRoot}; stableRuntimeAfter=${stableRuntimeAfterCleanup}; selective=${selectiveCleanupSucceeded}`);
