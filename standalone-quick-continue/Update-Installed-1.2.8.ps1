...
$commit = '23b1b4b86c326e03673eb6660d5ddbf418bdb9f6'
$baseUrl = "https://raw.githubusercontent.com/thatoneguydan/chatgpt-response-notifier/$commit/standalone-quick-continue"
$temp = Join-Path $env:TEMP ("ChatGPTQuickContinue-1.2.8-" + [Guid]::NewGuid().ToString('N'))
$backupRoot = Join-Path $env:LOCALAPPDATA 'ChatGPTQuickContinue\Backups'
$backup = Join-Path $backupRoot ("pre-1.2.8-" + (Get-Date -Format 'yyyyMMdd-HHmmss'))
$files = @('manifest.json', 'prompt-format.js', 'config.js', 'content-script.js', 'hover-edit-script.js', 'conversation-state.js', 'README.md')
...
try {
    ...
    if ([string]$downloadedManifest.version -ne '1.2.8') {
        throw "Downloaded Quick Continue manifest is version $($downloadedManifest.version), expected 1.2.8."
    }
    ...
    if ([string]$after.version -ne '1.2.8') {
        throw "Installed Quick Continue version is $($after.version), expected 1.2.8."
    }
