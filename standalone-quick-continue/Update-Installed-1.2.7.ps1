...
$commit = '1b5e9c7c5fb9a37103c75ed1c1559378b64f0d62'
$baseUrl = "https://raw.githubusercontent.com/thatoneguydan/chatgpt-response-notifier/$commit/standalone-quick-continue"
$temp = Join-Path $env:TEMP ("ChatGPTQuickContinue-1.2.7-" + [Guid]::NewGuid().ToString('N'))
$backupRoot = Join-Path $env:LOCALAPPDATA 'ChatGPTQuickContinue\Backups'
$backup = Join-Path $backupRoot ("pre-1.2.7-" + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    if ([string]$downloadedManifest.version -ne '1.2.7') {
        throw "Downloaded Quick Continue manifest is version $($downloadedManifest.version), expected 1.2.7."
    }
    ...
    if ([string]$after.version -ne '1.2.7') {
        throw "Installed Quick Continue version is $($after.version), expected 1.2.7."
    }
