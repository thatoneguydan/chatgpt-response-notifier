...
$commit = '425188e586660e52e1fd83ff96881e3904a8171d'
$baseUrl = "https://raw.githubusercontent.com/thatoneguydan/chatgpt-response-notifier/$commit/standalone-quick-continue"
$temp = Join-Path $env:TEMP ("ChatGPTQuickContinue-1.2.5-" + [Guid]::NewGuid().ToString('N'))
$backupRoot = Join-Path $env:LOCALAPPDATA 'ChatGPTQuickContinue\Backups'
$backup = Join-Path $backupRoot ("pre-1.2.5-" + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    if ([string]$downloadedManifest.version -ne '1.2.5') {
        throw "Downloaded Quick Continue manifest is version $($downloadedManifest.version), expected 1.2.5."
    }
    ...
    if ([string]$after.version -ne '1.2.5') {
        throw "Installed Quick Continue version is $($after.version), expected 1.2.5."
    }
