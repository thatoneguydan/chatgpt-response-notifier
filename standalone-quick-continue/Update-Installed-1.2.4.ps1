...
$backup = Join-Path $backupRoot ("pre-1.2.4-" + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    if ([string]$downloadedManifest.version -ne '1.2.4') {
        throw "Downloaded Quick Continue manifest is version $($downloadedManifest.version), expected 1.2.4."
    }
