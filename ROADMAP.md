...
Current installed version remains **v0.9.29**. The current loaded Glass acceptance candidate is `51061c8e3a8c6a0ab6dd327421b7d940c63ec83d`: canonical source through merged click-routing PR #98 plus the accepted PR #80 `Extension-v2` persistence lineage. Protected deploy and post-deploy verification passed. The operator had already completed the required same-version Reload; fresh read-only evidence proves the active worker changed from suffix `f9539c6f` to `b9af1a36`. PR #101 then removed this manual-Reload dependency for future same-version source replacements by adding exact build-source activation identity to canonical main at `5aa936ab7a0d85bd03d27de74e300ca363c90453`. Do not advance normal release/versioning past the deferred Windows reboot/login acceptance gate under #443.
...
| --- | --- | --- |
| 1 | Workstream 2/5 — Native Toast UX | **Step 1 source repair is complete through PR #98.** The exact live candidate is active; finish only physical cross-desktop click acceptance under #442. |
...
| Step 3/4 — Prove normal restart durability | Clean Chrome exit/reopen and Windows reboot/login retain the intended loaded extension and helper connection | **Chrome restart portion complete. Windows reboot/login portion deferred** until an ordinary safe restart is available. Test the current loaded combined source `51061c8e...`; no re-registration may manufacture the pass. |
...
| Step 2/2 — Accept cross-desktop click on Glass | Correct conversation becomes visible/focused without hang, crash or registration loss | **Active physical gate.** Use exact loaded source `51061c8e...`, whose active worker suffix is proven as `b9af1a36`. Cover the cross-desktop existing-tab click and explicit **Move this tab here** fallback; retain the existing same-desktop/minimized/duplicate/double-click/missing-script regression coverage. Preserve original tab/window and no unsolicited focus. Click acceptance must not manufacture recovery traffic; any page-affecting recovery remains governed by completed Workstream 4 traffic safety. |

v0.9.28's duplicate-window fallback is superseded by PR #98. Ordinary click now targets the exact existing conversation tab and treats presentation as complete only when that exact page is visible and focused. If Windows virtual-desktop isolation keeps it hidden, the toast remains and exposes explicit **Move this tab here**, which moves the same tab object with `chrome.windows.create({ tabId: ... })` rather than cloning its URL. Chrome still exposes no supported Windows-virtual-desktop switch destination. Keep native Win32 foreground/window enumeration and undocumented virtual-desktop switching retired.
...
- Current loaded Glass source is `51061c8e3a8c6a0ab6dd327421b7d940c63ec83d`, combining merged click-routing PR #98 with the accepted PR #80 `Extension-v2` persistence lineage. Protected deploy and read-only installed-source verification passed; fresh runtime evidence proves active worker suffix `b9af1a36`. PR #80 remains open until the deferred Windows reboot/login gate closes; do not replace this loaded candidate with persistence-only historical source.
- Runtime activation hardening: PR #101 / main `5aa936ab7a0d85bd03d27de74e300ca363c90453` stamps exact source identity into built extension payloads and compares it with helper install-state identity, so future same-version source replacements self-activate without an operator Reload. Empty/invalid source identity fails closed. This does not alter the fixed manifest key, extension ID or permanent root, and it does not require redeploying the already-active v0.9.29 acceptance candidate.

Implementation proceeds from this roadmap after reconciling owners/heads. Continue **ChatGPT Response Notifier → Workstream 2/5 — Native Toast UX → Stage 1/1 — Verified user-initiated click presentation → Step 2/2 — Accept cross-desktop click on Glass → Gate 1/1** under #442. Step 1 is complete in source and the exact live candidate is active; do not reopen Workstream 1 or repeat notification-delivery acceptance. Keep **ChatGPT Response Notifier → Workstream 3/5 — Installation, Release, and Acceptance → Stage 1/1 — Restore and prove installation persistence → Step 3/4 — Prove normal restart durability → Gate 1/1 — Windows reboot/login portion** deferred until an ordinary safe restart is available.