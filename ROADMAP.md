...
Roadmap retains **5 workstreams**. Workstreams 4 and 5 are source-complete. Workstream 3 is accepted through clean Chrome restart with only its physical Windows reboot/login portion deferred. The next deterministic source lane is Workstream 1; Workstream 2 follows until its physical cross-desktop acceptance boundary.

| Priority | Workstream | Immediate outcome |
| --- | --- | --- |
| 1 | Workstream 1/5 — Foundation and Conversation Identity | Complete the correlated trace, then prove observation/notification across hidden and background page states independently of recovery. |
...
| Step | Gate 1/1 | Tasks |
| --- | --- | --- |
| Step 1/2 — Complete the existing correlated trace | Each incident identifies its first missing boundary and action reason | Task 1/2: reuse v0.9.27 request/document/stream/page-query/lifecycle/helper evidence; add only missing decision evidence. Task 2/2: distinguish absent runtime, absent attachment, stale/throttled page, frozen/discarded page, veto and delivery failure. Recovery decision evidence from completed Workstream 4 should be reused rather than duplicated. |
| Step 2/2 — Accept hidden/background behavior | Each scenario independently proves observation and notification | Test foreground, inactive tab, unfocused/occluded, minimized and other Windows desktop; use disposable lifecycle controls for frozen/discarded cases. Retain the one v0.9.27 hidden notification as proof for that event only. Any recovery exercised during acceptance must still pass the completed Workstream 4 shared traffic guard. |

Next implementation position: **ChatGPT Response Notifier → Workstream 1/5 — Foundation and Conversation Identity → Stage 1/1 — Trustworthy observation across page states → Step 1/2 — Complete the existing correlated trace → Gate 1/1.**
...
- Current accepted live Glass source remains `0b4d045c92accc62c7914b2c0bcf2b0a33a71741`, combining accepted `Extension-v2` persistence with canonical traffic safety. Later canonical source work through main `248675e4a940621cae3fb6fedd6678a84ce8892a` is validated source-only and has not been installed by those lanes. PR #80 remains open until the deferred Windows reboot/login gate closes.

Implementation proceeds from this roadmap after reconciling owners/heads. Continue from **ChatGPT Response Notifier → Workstream 1/5 — Foundation and Conversation Identity → Stage 1/1 — Trustworthy observation across page states → Step 1/2 — Complete the existing correlated trace → Gate 1/1**. After deterministic Workstream 1 source work, continue **ChatGPT Response Notifier → Workstream 2/5 — Native Toast UX → Stage 1/1 — Verified user-initiated click presentation → Step 1/2 — Bound and verify the browser route → Gate 1/1**. Keep **ChatGPT Response Notifier → Workstream 3/5 — Installation, Release, and Acceptance → Stage 1/1 — Restore and prove installation persistence → Step 3/4 — Prove normal restart durability → Gate 1/1 — Windows reboot/login portion** deferred until an ordinary safe restart is available.