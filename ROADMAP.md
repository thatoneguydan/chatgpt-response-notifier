...
Current installed version remains **v0.9.29**. Source completed after the accepted live combined build has **not** been redeployed by the Workstream 4/5 or Workstream 5/5 source lanes. Do not advance normal release/versioning past the deferred Windows reboot/login acceptance gate under #443, and do not overwrite the accepted `Extension-v2` runtime merely to exercise source-only work.
...
Roadmap retains **5 workstreams**. Workstreams 4 and 5 are source-complete. Workstream 1 Step 1 is complete through PR #93 / main `984e93c52b177a275d72bc3c033d84b40de3b68a`; Step 2 now has a fresh state-specific failure under DevelopmentInfrastructure #478: same-desktop unfocused and minimized delivery pass, but another Windows virtual desktop does not notify on the current live runtime. Workstream 3 is accepted through clean Chrome restart with only its physical Windows reboot/login portion deferred. Workstream 2 follows after Workstream 1.

| Priority | Workstream | Immediate outcome |
| --- | --- | --- |
| 1 | Workstream 1/5 — Foundation and Conversation Identity | Correlated trace is complete; diagnose and repair the fresh cross-Windows-virtual-desktop notification failure under #478. Same-desktop unfocused and minimized delivery are accepted. |
| 2 | Workstream 2/5 — Native Toast UX | Finish bounded click verification source work, then pass physical cross-desktop acceptance. |
...
| Step 3/4 — Prove normal restart durability | Clean Chrome exit/reopen and Windows reboot/login retain the intended loaded extension and helper connection | **Chrome restart portion complete. Windows reboot/login portion deferred** until an ordinary safe restart is available. Test the current accepted live combined source `0b4d045...`; no re-registration may manufacture the pass. |
...
| Step 2/2 — Accept hidden/background behavior | Each scenario independently proves observation and notification | **Current acceptance failure isolated to another Windows virtual desktop.** Foreground/inactive deterministic proof remains; fresh physical tests show another app focused on the same desktop works and Chrome minimized works. The current live runtime does not notify when Chrome/ChatGPT is on another Windows virtual desktop. #439's older v0.9.27 cross-desktop pass is historical-only evidence and does not close the current gate. Diagnose the first missing boundary from retained evidence before selecting a repair. Canonical checkpoint: `docs/BACKGROUND-OBSERVATION-ACCEPTANCE-2026-09-17.md`; owner #478. |

Next implementation position: **ChatGPT Response Notifier → Workstream 1/5 — Foundation and Conversation Identity → Stage 1/1 — Trustworthy observation across page states → Step 2/2 — Accept hidden/background behavior → Gate 1/1 — diagnose the fresh cross-Windows-virtual-desktop notification failure from retained evidence, then repair only the first proven missing boundary.**
...
- Current accepted live Glass source remains `0b4d045c92accc62c7914b2c0bcf2b0a33a71741`, combining accepted `Extension-v2` persistence with canonical traffic safety. Later canonical source work through main `984e93c52b177a275d72bc3c033d84b40de3b68a` is validated source-only and has not been installed by those lanes. The passive coded-completion → durable-notification files listed in `docs/BACKGROUND-OBSERVATION-ACCEPTANCE-2026-09-17.md` are byte-identical between those commits, allowing a successful minimized-window notification on the accepted live runtime to serve as positive evidence for that unchanged path. PR #80 remains open until the deferred Windows reboot/login gate closes.

Implementation proceeds from this roadmap after reconciling owners/heads. Continue **ChatGPT Response Notifier → Workstream 1/5 — Foundation and Conversation Identity → Stage 1/1 — Trustworthy observation across page states → Step 2/2 — Accept hidden/background behavior → Gate 1/1** under #478 by diagnosing the fresh cross-Windows-virtual-desktop notification failure. Do not repeat the now-passed minimized or same-desktop unfocused tests, and do not treat #439's historical cross-desktop pass as current acceptance. After that pass, continue **ChatGPT Response Notifier → Workstream 2/5 — Native Toast UX → Stage 1/1 — Verified user-initiated click presentation → Step 1/2 — Bound and verify the browser route → Gate 1/1**. Keep **ChatGPT Response Notifier → Workstream 3/5 — Installation, Release, and Acceptance → Stage 1/1 — Restore and prove installation persistence → Step 3/4 — Prove normal restart durability → Gate 1/1 — Windows reboot/login portion** deferred until an ordinary safe restart is available.