...
Roadmap retains **5 workstreams**. Workstreams 4 and 5 are source-complete. Workstream 1 Step 1 is complete through PR #93 / main `984e93c52b177a275d72bc3c033d84b40de3b68a`; Step 2 has one remaining physical minimized-window acceptance under DevelopmentInfrastructure #478. Workstream 3 is accepted through clean Chrome restart with only its physical Windows reboot/login portion deferred. Workstream 2 follows Workstream 1 until its physical cross-desktop click acceptance boundary.

| Priority | Workstream | Immediate outcome |
| --- | --- | --- |
| 1 | Workstream 1/5 — Foundation and Conversation Identity | Correlated trace is complete; finish the single remaining minimized-window physical notification acceptance under #478. |
...
| Step 2/2 — Accept hidden/background behavior | Each scenario independently proves observation and notification | **Acceptance matrix complete except minimized Chrome.** Foreground/inactive behavior has deterministic source proof; #439 retains the real hidden/unfocused/occluded/other-Windows-desktop notification pass; frozen/discarded behavior is accepted as explicit safe defer/unobservable handling rather than manufactured terminal notification. No retained minimized-window proof exists. Canonical checkpoint: `docs/BACKGROUND-OBSERVATION-ACCEPTANCE-2026-09-17.md`; owner #478. |

Next implementation position: **ChatGPT Response Notifier → Workstream 1/5 — Foundation and Conversation Identity → Stage 1/1 — Trustworthy observation across page states → Step 2/2 — Accept hidden/background behavior → Gate 1/1 — minimized-window physical acceptance only.**
...
Implementation proceeds from this roadmap after reconciling owners/heads. Workstream 1 is blocked only on **ChatGPT Response Notifier → Workstream 1/5 — Foundation and Conversation Identity → Stage 1/1 — Trustworthy observation across page states → Step 2/2 — Accept hidden/background behavior → Gate 1/1 — minimized-window physical acceptance** under #478; do not repeat the already-accepted other-desktop hidden test. After that pass, continue **ChatGPT Response Notifier → Workstream 2/5 — Native Toast UX → Stage 1/1 — Verified user-initiated click presentation → Step 1/2 — Bound and verify the browser route → Gate 1/1**. Keep **ChatGPT Response Notifier → Workstream 3/5 — Installation, Release, and Acceptance → Stage 1/1 — Restore and prove installation persistence → Step 3/4 — Prove normal restart durability → Gate 1/1 — Windows reboot/login portion** deferred until an ordinary safe restart is available.