...
The first live unified-control pass also exposed a false footer-repair loop: an exact visible terminal footer such as `[GITHUB_STATUS: BLOCKED_HUMAN]` could be rendered in a later assistant content block while the status/monitor readers inspected only the first `.markdown`/prose block. The readers now aggregate all top-level rendered assistant blocks before strict parsing. The monitor also records fence-aware exact terminal-footer evidence and refuses to classify a response as `status-missing` while such evidence is visible, so a parser/rendering mismatch fails quiet instead of sending a repair prompt. Shared status parser and continuation-policy globals are runtime-versioned so hot reinjection replaces stale copies left in already-open ChatGPT tabs.

The acceptance candidate advances to v0.9.1 so activation uses the normal versioned lifecycle. Setup installs both helper and extension files and starts the new helper; an already-running older extension worker sees the higher installed version from the helper and calls `chrome.runtime.reload()` itself. Managed public updates already download and verify the bundle, install both components, register/start the replacement helper windowlessly, and trigger that same extension-runtime reload. Therefore normal versioned updates require no separate plugin/helper installation and no manual `chrome://extensions` reload. An unreleased acceptance candidate still requires one Setup launch until the live gate authorizes publication.
...
These repairs remain inside **ChatGPT Response Notifier → Workstream 5/5 → Stage 4/4 → Step 1/1 → Gate 1/1 → Task 2/3** as prerequisite work for live browser acceptance. No public release or PR merge is authorized by deterministic proof alone.
...
- Task 1/3 — Exact candidate component/architecture tests and Windows helper/installer build validation. **Complete for the repaired source line, including same-version in-place replacement while the old helper is running.**
...
- Task 1/3 — Reconcile installed extension/helper source identity and PR #33. **Complete. Live read-only evidence identified installed v0.9.0 source `bdcea39eaba9b960ce3579016c9b1308585d5e8b`; this is older than the unified-control source line and established the need for exact-candidate replacement.**
...
- Task 2/3 — Versioned UI/request observer and classifier while preserving raw upstream detector bytes. **Complete. Footer observation now spans all rendered assistant content blocks, and exact terminal-footer evidence vetoes automatic format repair even if strict parsing cannot classify the rendered shape.**
...
- Task 1/3 — Deterministic architecture/state-machine coverage: 100+ continuation cycles, blockers, simultaneous chats, outbox/restart, unified state, sticky Pause, START, quiet close/duplicate ownership, revision conflicts, installer lifecycle, split-rendered footer observation, and automatic versioned activation. **Complete in source; exact final-head Glass proof required before the next live install. The installer regression gate performs two same-version installs into one isolated root while the first helper is running and requires exact source identity plus helper handshake after replacement.**
- Task 2/3 — Real supported ChatGPT acceptance: one recognized/enabled build chat with bounded actions; combined control; auto-recognition; sticky Pause/Resume; quiet close/duplicate ownership; inactive/minimized tab; SPA navigation; active-user/draft safeguards; helper restart; automatic extension hot attachment/update; freeze/resume; adapter false-stall/recovery evidence; exact extension/helper/Chrome identity. **Pending exact v0.9.1 candidate proof/install and live acceptance. Do not manufacture repeated server failures or usage-limit probes.**
...
- Exact valid terminal-footer evidence suppresses format repair even if the strict parser cannot classify the current rendered DOM shape; a visible valid footer must never cause the notifier to ask for another footer.
- Hot reinjection must replace stale parser/policy globals when runtime versions differ while remaining idempotent within the same runtime version.
- Versioned installs/managed updates activate the replacement helper windowlessly and reload the extension runtime automatically when the installed extension version advances. Manual Chrome extension reload is not a normal update step.
...
Active implementation point: **ChatGPT Response Notifier → Workstream 5/5 — Status Contract and Bounded Recovery → Stage 4/4 — Prove and Roll Out → Step 1/1 — Validate the real background workflow → Gate 1/1 — Exact-source evidence supports enabling recovery → Task 2/3 — prove the exact v0.9.1 source on Glass, install the resulting single Setup candidate, verify automatic extension-runtime activation and installed source identity, then continue real Chrome acceptance.**

Known live state before v0.9.1: the user successfully installed/reloaded the repaired v0.9.0 candidate manually, after an earlier same-version installer failure was fixed. That live pass then exposed the repeated `BLOCKED_HUMAN` format-repair defect described above. The v0.9.1 candidate fixes split-rendered footer observation, adds a valid-footer repair veto, versions hot parser/policy globals, and exercises the normal automatic version-advance activation path. PR/issue tracking owns the exact latest candidate commit/artifact/hash. No public release or PR merge before Task 2 passes.
