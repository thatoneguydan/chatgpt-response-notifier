...
- when the standalone Quick Continue toolbar is present, adds a small monitoring-state light immediately before **Continue**: gray for Monitor/ready, green for active/Pause, and amber for Resume/paused or recovery-guarded; clicking it uses the same verified Monitor → Pause → Resume control path as the popup;
