...
- when the standalone Quick Continue toolbar is present, adds a small monitoring-state light immediately before **Continue**: gray while inactive/paused/guarded and saturated green while active; clicking it uses the same verified Monitor → Pause → Resume control path as the popup;
- while automation is active, shows the persisted watchdog countdown and remaining automatic Continue budget in small text above the Quick Continue controls, including an explicit exhausted state after the three-send budget is spent;
